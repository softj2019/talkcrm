...
(function( $ ) {
    ...
    $.fn.drawr = function( action, param ) {
		...
		var tspImg="../assets/img/dummy_img_L.png";
		...
		plugin.get_mouse_data = function (event,relativeTo,scrollEl) {//// body 이벤트이지만 다른 요소와 관련하여 나중에 추가로 확장됩니다.
...
//        	if(this.settings.enable_tranparency==true) $(this).css({"background-image" : "url(" + tspImg + ")"});
        ...
        plugin.create_toolbox = function(id,position,title,width){
			...
			$(toolbox).css({
				"position" : "absolute", "z-index" : -1, "cursor" : "move", "width" : width + "px", "height" : "auto", "color" : "#fff",
